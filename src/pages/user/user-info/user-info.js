...
import userInfoTemplateSource from "./user-info.handlebars?raw";
...
export function renderUserInfoForm() {
  ...
  renderChangeDataBtn.addEventListener('click', (e) => {
    e.preventDefault()
    import('../user-change-data/user-change-data.js').then(({renderUserDataForm}) => {
        renderUserDataForm()
    })
  })
  ...
  renderChangePassBtn.addEventListener('click', (e) => {
    e.preventDefault()
    import('../user-change-password/user-change-password.js').then(({renderUserPassForm}) => {
        renderUserPassForm()
    })
  })
